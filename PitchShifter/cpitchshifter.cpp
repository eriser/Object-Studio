...
CPitchShifter::CPitchShifter()
{
}

void CPitchShifter::smbPitchShift(float pitchShift, long numSampsToProcess, long fftFrameSize, long osamp, float sampleRate, float *indata, float *outdata) {
...
void CPitchShifter::Init(const int Index, void *MainWindow) {
    ModFactor=0;
    gRover = false;
    gInit = false;
    m_Name="PitchShifter";
...
float *CPitchShifter::GetNextA(const int ProcIndex) {
    float* InSignal=FetchA(jnIn);
    float CurrMod=Fetch(jnModulation);
    float TempTune=pow(2.0,CurrMod*ModFactor);

    float Modulation=TempTune;
    if (!InSignal) return NULL;
    smbPitchShift(PitchShift*Modulation, m_BufferSize, FrameSize,OverSampling, m_Presets.SampleRate, InSignal, AudioBuffers[ProcIndex]->Buffer);
    return AudioBuffers[ProcIndex]->Buffer;
}

void CPitchShifter::CalcParams() {
    PitchShift = pow(2., m_ParameterValues[pnShift]/12.);	// convert semitones to factor
    OverSampling=1 << m_ParameterValues[pnOverSampling];
    FrameSize=128 << m_ParameterValues[pnFrameSize];
